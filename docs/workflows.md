...
End-to-end workflows for all project scenarios. See the [README](../README.md) for installation and overview.
...
The `project-setup-wizard` is the entry point for every project session. It detects your project's current state and delivers the exact workflow to follow.
...
The `project-setup-advisor` is a lighter alternative. It scans the project and outputs a workflow recommendation without asking questions.
