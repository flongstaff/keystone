...
# The none-scenario block spans lines 271-319. All 3 conditional recommendation branches
# must each include all 3 menu options.
BMAD_COUNT=$(sed -n '271,319p' "$REPO_ROOT/skills/wizard.md" | grep -c "Start with BMAD" || true)
GSD_COUNT=$(sed -n '271,319p' "$REPO_ROOT/skills/wizard.md" | grep -c "Start with GSD" || true)
QT_COUNT=$(sed -n '271,319p' "$REPO_ROOT/skills/wizard.md" | grep -c "Quick task" || true)
